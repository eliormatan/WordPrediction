Elior Matan 308470715
Ibrahim Awad 318260585
...
The Manager starts a thread pool, and listens to an sqs queue for messages from local apps (multiple local apps are supported) until he receives a message with terminate. The manager creates two runnable tasks, (one to handle incoming messages from local app, and another to handle returning done image ocr tasks from workers), and executes them with the executor.
...
Runtime: using the provided input.txt, n = 5, took 5 minutes.


Security:
The manager and all of the workers get temporary credentials from their IAM role. Therefore, we don't transfer them credentials at all.
...
Persistence:
In our implementation we take advantage of the SQS visibility time-out, if a worker does not finish his job, the task message he received will return to the sqs queue, and another worker will receive it.
TO BE CONTINUED
